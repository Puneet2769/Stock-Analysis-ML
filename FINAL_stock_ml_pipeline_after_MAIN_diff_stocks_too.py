...
def main():
    tickers = [
        ...
        # "GOLDBEES.NS"
        "SBIN.NS"
        # add or remove anything you want here
    ]

    for t in tickers:
        print("\n" + "=" * 60)
        print(f"Running pipeline for {t}")
        print("=" * 60)
        run_for_ticker(t)
...
TICKER = "RELIANCE.NS" / "TCS.NS" / "HDFCBANK.NS" etc.

Data length

PERIOD = "5y"

You could try "3y", "10y" etc (more data vs regime changes).

Interval

INTERVAL = "1d"

You could try "1h", "1wk" later, but then all indicators, horizons, and backtests need re-thinking.

Train/test split

TRAIN_FRACTION = 0.8

You already experimented with 0.7 / 0.9 etc.

3.2 Label / target knobs

Horizon

1-day: Tomorrow_Close

5-day: Close_5d
You could add:

10-day, 20-day horizons, etc.

Definition of “Up” / “Down”
Right now:

Target_Up = 1 if future_close > current_close else 0

You could introduce thresholds, like:

Only call it “Up” if future_return > 0.3%

Call small moves “No Trade” (ignore)

Regression target

Now: Return_1d_future = (Tomorrow_Close - Close) / Close
You could:

Use 5-day future return instead
...
SMA_10, SMA_20 → try 5/20, 20/50, etc.
...
Return_5d, Return_3d_mean, Return_7d_mean

Volatility:

Volatility_3d, Volatility_7d, Volatility_10d

Bollinger:

window = 20, multiplier = 2

could play with 10, 30; 1.5σ, 2.5σ
...
Each of these window sizes is a knob, but tuning all of them together is a combinatorial explosion and will overfit easily.

3.4 Model knobs (RandomForest)

For classifier & regressor:
...
max_features (not set, but could be changed)

random_state (for reproducibility, not performance)

You can grid-search a few values, for example:
...
But do this properly (see section 4).

3.5 Strategy knobs (how you trade the signal)

1-day strategy (classifier):

Enter if Pred_Class == 1 (current)

You could change:

Only enter if Pred_Class == 1 AND RSI_14 between 40 and 60

Only enter if Pred_Class == 1 AND SMA_10 > SMA_20

Combine with MACD (MACD_Line > MACD_Signal)

Require BB_Width > some threshold (high volatility only)
...
These thresholds (40, 70, trend condition) are knobs too.

Regression-based strategy (if you want):

Enter long if predicted return > X

Go flat if between –X and +X

(If you allow shorts) short if predicted return < –X

So entry thresholds become strategy tuning parameters.
...
Add cost per trade (e.g. 0.1% per buy/sell)

Use a fixed fraction of capital per trade (e.g. 50%)

Enforce a max number of open trades (more relevant in portfolio setting)

But again, if the signal is weak, better backtest rules won’t create magic edge.
'''
#endregion

# region Instruction For Future

'''

🔹 Step 1 – Fix a test set and never touch it

Example:

Use the last 20% of the time series as the final test.

Do not keep changing where the split is just to get higher accuracy.

All tuning must be evaluated on train (or train + validation), not on test.

🔹 Step 2 – Choose a small set of knobs for one experiment

Don’t tune everything at once.

Examples of one experiment:

Experiment A: only tune max_depth, min_samples_leaf, n_estimators

Experiment B: only tune RSI window and SMA windows

Experiment C: only tune 5-day strategy filters (RSI thresholds, SMA condition)

Keep each experiment small and focused.

🔹 Step 3 – Use a loop to scan values and store results

In code (later, if you implement):

For each combination of parameters:

Rebuild features / labels if needed

Train on train part

Evaluate on a validation part (subset of train, or walk-forward)

Store:

validation accuracy

maybe PnL on validation backtest

At the end, you:

Plot param value vs accuracy,

Or param value vs PnL.

That will give you the “graph of all data” you’re imagining.

Important:

You must pick the best parameters based on validation, not test.
Then do one final run on test to see how it really performs.

🔹 Step 4 – Be suspicious of small gains

If accuracy goes from:

0.50 → 0.53 on a small sample,
it might be:

randomness, not real signal.

For markets, stability is more important than squeezing a 1–3% accuracy bump on one test split.
